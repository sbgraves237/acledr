...
(@) Read and summarize `ACLED` data in various ways. *[this vignette]*
...
(@) Merge `ACLED` and `WB` data. 
(@) Build static models of the relationships between `ACLED` and `WB` variables. 

## Extract, download and read `ACLED` data 

To make everything comparable, we downloaded events and fatalities from ACLED Explorer by country and event type for each month starting with 2021-01 as `*.csv` files. We moved these two files in the current working directory and renamed them `acled_aggregated_2021events.csv` and `acled_aggregated_2021fat.csv`, respectively. 

```{r}
#| label: csvFiles
#extdata <- pkgload::shim_system.file('extdata', package='acledr')
extdata <- fs::path_package('extdata', package='acledr')
csvFiles <- dir(extdata, pattern='\\.csv$')
(deathFile <- dir(extdata, pattern='2021fat\\.csv$', 
                  full.names = TRUE))
(eventFile <- dir(extdata, pattern='2021events\\.csv$', 
                  full.names = TRUE))
```
