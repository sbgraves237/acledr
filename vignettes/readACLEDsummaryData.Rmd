...
- `noSearch` to stop before `dir(extdata)`, which should contain data files to be read. 
- `noRead` to stop before attempting to read files that were found. 
- `all` to run the entire vignette. This works on my local computer but not on GitHub Action. 
...
if(run != 'noSearch'){
  ...
  (eventFile <- dir(extdata, pattern='events\\.csv$', 
                  full.names = TRUE))
  if(max(nchar(deathFile))<1){
    stop('deathfile = ', deathFile, ' not found.')
  }
  if(max(nchar(eventFile))<1){
    stop('eventfile = ', eventFile, ' not found.')
  }
}
```
