...
Then I clicked "Download options" and selected "Excel". That downloaded a file named `P_Popular Indicators.xlsx`, which I moved to the working directory and changed the name by adding the current data in compressed ISO 8601 format as 20231113. 
...
```{r extdata}
run <- c("noSearch", "noDir", "noRead", 'read', "allButSave", "all")[3]
...
if(run %in% c("noRead", 'read', "allButSave", "all")){
  ...
  (WBfiles <- grep('^P_', xlsFiles, value=TRUE))
...
```{r CtryCodeChk}
if(run %in% c("allButSave", "all")){
  dim(Ctry_code <-  with(WBdat, table(Country, CtryCode)))
  sum(Ctry_code!=0)
}
```
...
```{r  CtryCode}
if(run %in% c("allButSave", "all")){
  str(WBctryCode <- WBdat[1:217, 3:4])
